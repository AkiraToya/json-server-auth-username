...
beforeEach(async () => {
	...
	bearer = { Authorization: `Bearer ${res.body.token}` }
})
...
test('[HAPPY] create another user after setting guards', async () => {
	const res = await rq
		.post('/users')
		.send({ username: 'arthur', password: '1234' })
		.expect(201)

	const otherBearer = { Authorization: `Bearer ${res.body.token}` }
